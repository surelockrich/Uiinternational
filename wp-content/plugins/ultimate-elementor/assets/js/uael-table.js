( function( $ ) {
	...
	var WidgetUAELTableHandler = function( $scope, $ ) {
		...
		// Show entries select
		var show_entry = $( '.elementor-element-' + node_id + ' #' + uael_table_id[0].id ).attr( 'show-entry' );
		...
		var sort_table = $( '.elementor-element-' + node_id + ' #' + uael_table_id[0].id ).attr( 'sort-table' );
		if ( 'yes' == sort_table ) {
			$( '.elementor-element-' + node_id + ' #' + uael_table_id[0].id + ' th' ).css({'cursor': 'pointer'});
			$( uael_table_name ).click(function() {
				var col_number = $( this ).attr( 'data-sort' );
